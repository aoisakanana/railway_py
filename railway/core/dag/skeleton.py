...
class SkeletonSpec:
    ...
    entrypoint: str
    is_exit_node: bool
    # YAML の明示 function: に対応する関数名。空文字は未指定（node_name の leaf を使用）
    function_name: str = ""
...
def _spec_function_name(spec: SkeletonSpec) -> str:
    """スペックから生成する関数名を解決（純粋関数）。

    明示 function_name があればそれを、なければ node_name の leaf を使う。
    """
    return spec.function_name or spec.node_name.split(".")[-1]


def _generate_node_header(func_names: tuple[str, ...]) -> str:
    """モジュールヘッダ（docstring + import）を生成（純粋関数）。"""
    title = " / ".join(func_names)
    return f'''"""{title} ノード
...
def _generate_node_body(func_name: str) -> str:
    """1 関数分の Contract + ノード関数を生成（純粋関数）。"""
...
def generate_regular_node_content(spec: SkeletonSpec) -> str:
    ...
    """
    return generate_module_content((spec,))


def generate_module_content(specs: tuple[SkeletonSpec, ...]) -> str:
    """同一 module を共有する複数ノードのスケルトンを 1 ファイル分生成（純粋関数）。

    v0.13.26 Issue 02: 同一 module に複数の `function:` が定義されている場合、
    必要な全関数を 1 ファイルに生成する。

    Args:
        specs: 同一 module_path を持つスケルトン仕様のタプル（1 件以上）

    Returns:
        生成された Python コード文字列
    """
    func_names = tuple(_spec_function_name(s) for s in specs)
    parts = [_generate_node_header(func_names)]
    parts.extend(_generate_node_body(name) for name in func_names)
    return "".join(parts)
