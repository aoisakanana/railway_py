"""v0.13.10 -> v0.13.10rc3 マイグレーションテスト."""
from pathlib import Path

import pytest

from railway.migrations.definitions.v0_13_10_to_v0_13_11 import (
...
class TestMigrationDefinition:
    ...
    def test_migration_is_registered(self) -> None:
        migration = find_migration("0.13.10", "0.13.11")
        assert migration is not None

    def test_migration_versions(self) -> None:
        assert MIGRATION_0_13_10_TO_0_13_11.from_version == "0.13.10"
        assert MIGRATION_0_13_10_TO_0_13_11.to_version == "0.13.11"

    def test_migration_path_includes_new(self) -> None:
    ...
    def test_code_guidance_mentions_mypy(self) -> None:
        assert any(
            ...
            for g in MIGRATION_0_13_10_TO_0_13_11.code_guidance
        )

    def test_post_migration_includes_sync(self) -> None:
        assert any(
            "sync" in cmd
            for cmd in MIGRATION_0_13_10_TO_0_13_11.post_migration_commands
        )

    def test_warnings_mention_nested_path_change(self) -> None:
        assert any(
            "sub.deep.process" in w or "ネスト" in w
            for w in MIGRATION_0_13_10_TO_0_13_11.warnings
        )

    def test_file_changes_include_init_py(self) -> None:
        paths = [c.path for c in MIGRATION_0_13_10_TO_0_13_11.file_changes]
...
class TestMigrationExecution:
    ...
    def test_tests_nodes_init_py_created(self, tmp_path: Path) -> None:
        ...
        for change in MIGRATION_0_13_10_TO_0_13_11.file_changes:
