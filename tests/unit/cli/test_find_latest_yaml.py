...
class TestFindLatestYaml:
    ...
    def test_exact_match(self, tmp_path: Path) -> None:
        ...
        result = find_latest_yaml(tmp_path, "alert")
    ...
    def test_underscore_entry_name(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "alert_hoge_20260101000000.yml").touch()
        (tmp_path / "alert_20260101000000.yml").touch()
        result = find_latest_yaml(tmp_path, "alert_hoge")
        assert result is not None
    ...
    def test_non_yml_files_ignored(self, tmp_path: Path) -> None:
        """非 YML ファイルは無視されること。"""
        (tmp_path / "alert_20260101000000.txt").touch()
        (tmp_path / "alert_20260101000000.yaml").touch()
        result = find_latest_yaml(tmp_path, "alert")
        assert result is None
    ...
    def test_similar_prefix_not_matched(self, tmp_path: Path) -> None:
        ...
        # alert_hoge_20260101.yml は "alert" にマッチしてはいけない
        (tmp_path / "alert_hoge_20260101000000.yml").touch()
        result = find_latest_yaml(tmp_path, "alert")
        assert result is None
